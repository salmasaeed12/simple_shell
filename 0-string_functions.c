...
int _strlen(char *s)
{
    ...
    if (!s)
        return (0);
    // when pointer of s increase i will increase then return length of s
    while (*s++)
        i++;
    return (i);
}
 ...
 *
 *Return: negative value if str1 > str2 ,positive if str1 < str2 ,zero if they are equaled
 */
...
{
    char *r = dest;
    // The first value concatenate with two value
    while (*dest)
        dest++;
    while (*src)
        *dest++ = *src++;
